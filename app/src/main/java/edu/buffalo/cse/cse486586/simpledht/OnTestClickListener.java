...
public class OnTestClickListener implements OnClickListener {
	...
	private class Task extends AsyncTask<Void, String, Void> {
		...
		private void testDelete(){
			for (int i =0 ; i< TEST_CNT;i++){
				String toDelete = (String) mContentValues[i].get(KEY_FIELD);
				publishProgress("Deleting "+ toDelete+"\n");
				mContentResolver.delete(mUri,toDelete,null);
			}
			publishProgress(mContentResolver.toString());
		}
